...
namespace Calculator.Tests
{
    public class CalculatorTests
    {
        ...
        [InlineData("//;\n1;9", 10)]
        [InlineData("//*\n5*5*1*2", 13)]
        public void Add_Changed_One_Sign_Delimiter(string input, int expectedResult)
        {
            var calculator = new Calculator();
            int result = calculator.Add(input);
            Assert.Equal(expectedResult, result);
        ...
        [InlineData("1045", 0)]
        [InlineData("1002,5",5)]
        [InlineData("1002,3,67", 70)]
        public void Add_Throws_Numbers_Bigger_Then_Thousand_Specifying_Them(string input, int normalSum)
        {
            var calculator = new Calculator();
            var result = calculator.Add(input);

            Assert.Equal(result, normalSum);
        }

        [Theory]
        [InlineData("//[***]\n2***18", 20)]
        [InlineData("//[::]\n3::13::17::4", 37)]
        public void Add_Changed_Multi_Sign_Delimiter(string input, int expectedResult)
        {
            var calculator = new Calculator();
            int result = calculator.Add(input);
            Assert.Equal(expectedResult, result);
        ...
        [InlineData("//[*][^]\n5*16^12", 33)]
        [InlineData("//[:][$][&]\n18$14:17&8\n18", 75)]
        public void Add_Changed_One_Sign_Delimiters(string input, int expectedResult)
        {
            var calculator = new Calculator();
            int result = calculator.Add(input);
            Assert.Equal(expectedResult, result);
